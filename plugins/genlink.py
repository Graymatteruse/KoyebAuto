...
async def gen_link_batch(bot, message):
    if " " not in message.text:
        return await message.reply("𝚄𝚂𝙴 𝙲𝙾𝚁𝚁𝙴𝙲𝚃 𝙵𝙾𝚁𝙼𝙰𝚃.\n𝙴𝚇𝙰𝙼𝙿𝙻𝙴 ›› <code>/batch https://telegram.me/FHDmovies24x7/10 https://telegram.me/FHDmovies24x7/20</code>.")
    links = message.text.strip().split(" ")
    if len(links) != 3:
        return await message.reply("Use correct format.\nExample <code>/batch https://telegram.me/FHDmovies24x7/10 https://telegram.me/FHDmovies24x7/20</code>.")
    ...
    if chat_id in FILE_STORE_CHANNEL:
        ...
        return await sts.edit(f"<b>🎰 Here Is Your Link👇 https://telegram.me/{temp.U_NAME}?start=DSTORE-{b_64}</b>")
